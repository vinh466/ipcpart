...
<script setup lang="ts">
  ...
  const slidesImg = [
    ...
    'public/image/slices/slide-7.png',
  ...
  onMounted(async () => {
    cpuData.value = await productService.getProducts('cpu');
    mainboardData.value = await productService.getProducts('mainboard');
    ramData.value = await productService.getProducts('ram');
    monitorData.value = await productService.getProducts('monitor');
    mouseData.value = await productService.getProducts('mouse');
    storagedriveData.value = await productService.getProducts('storage drive');
    videocardData.value = await productService.getProducts('video card');
  });
</script>
