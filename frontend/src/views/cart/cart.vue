<template>
  <div class="">
    <div class="container">
      ...
      <div class="cart">
        <div class="cart-detail" :class="{ isLoading: classBlind.isLoading }">
          ...
          <table class="cart-table" cellspacing="0">
            ...
            <tbody>
              <CartItem
                ...
                :key="index"
                :cart-item="cartItem"
                @on-amount-change="handleAmountChange"
              />
              ...
              <tr>
                <td colspan="6" class="">
                  <div class="actions">
                    ...
                    <input type="hidden" id="cart-nonce" />
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div
          ...
          :class="{ isLoading: classBlind.isLoading }"
        >
          <div class="loading-icon">
            <font-awesome-icon icon="fa fa-spinner" />
          </div>
              ...
              <tbody>
                <tr class="order-subtotal">
                  ...
                  <td data-title="Tạm tính">
                    <span
                      class="price-amount"
                      :class="{ isChange: classBlind.isChangeSubTotal }"
                      >{{ formatVND(cartSubTotal) }}</span
                    >
                  </td>
                </tr>
...
<script setup lang="ts">
  ...
  import type { OrderItem } from '@/types/api/products/order';
  ...
  const timer = {
    subTotalChangeClass: <null | ReturnType<typeof setTimeout>>null,
    isLoadingChange: <null | ReturnType<typeof setTimeout>>null,
  };
  ...
  const { cartItems, count } = storeToRefs(cartStore);

  const cartTotal = computed(() => {
    let total = 0;
    cartItems.value.forEach((cartItem, index) => {
      total += cartItem.quantity * cartItem.price * 1000;
    });
    return total;
  });
  ...
  const classBlind = reactive({
    isChangeSubTotal: false,
    isLoading: false,
  });
  ...
  function handleAmountChange(
    ...
    classBlind.isLoading = true;
    timer.isLoadingChange && clearTimeout(timer.isLoadingChange);
    timer.isLoadingChange = setTimeout(() => {
      cartItems.value.forEach((cartItem, index) => {
        if (cartItem.productId == cartId) {
          cartItem.quantity = newAmount;
          classBlind.isChangeSubTotal = true;

          timer.subTotalChangeClass && clearTimeout(timer.subTotalChangeClass);
          timer.subTotalChangeClass = setTimeout(() => {
            classBlind.isChangeSubTotal = false;
          }, 500);
        }
      });

      classBlind.isLoading = false;
    }, 500);
  }
</script>
